...
@Config
object IntakeAutoDatalog100Config {
    ...
    @JvmField var MAX_DRIVE_POWER = 1.00
    @JvmField var MIN_DRIVE_POWER = 0.30
    @JvmField var RAMP_UP_DISTANCE_METERS = 0.40
    @JvmField var RAMP_DOWN_DISTANCE_METERS = 0.85
...
/**
 * Automated Intake Datalogger at Speed 1.00 (Autonomous Max Speed).
 *
 * Sequence:
 ...
 * 3. Executes 3.0m Forward and 3.0m Backward runs at full throttle (100% power) for the configured number of cycles.
 * 4. Logs all Intake, electrical, slide extension, sensors, and drivetrain kinematics to CSV.
 */
@Autonomous(name = "Intake Auto Datalog 3m (Speed 1.00)", group = "Autonomous")
class IntakeAutoDatalog100OpMode : OpMode() {
    ...
    override fun init() {
        ...
        telemetry.addLine("Intake 3m Auto Datalogger (Speed 1.00 Max)")
        telemetry.addData("Test Plan", "Extend Slides -> Intake Spin -> %.1fm Fwd/Bwd x %d cycles (%d segments)",
            IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS,
            IntakeAutoDatalog100Config.TOTAL_CYCLES,
            IntakeAutoDatalog100Config.TOTAL_CYCLES * 2
        )
        telemetry.addData("Speed & Power", "Max=%.2f, Min=%.2f",
            IntakeAutoDatalog100Config.MAX_DRIVE_POWER, IntakeAutoDatalog100Config.MIN_DRIVE_POWER
        )
    ...
    override fun start() {
        ...
        if (IntakeAutoDatalog100Config.AUTO_START_ON_PLAY && phase == TestPhase.IDLE) {
            beginTest()
        }
    }
    ...
    override fun loop() {
        ...
        val btnA = (gamepad1.a && !gamepad1.start) || (gamepad2.a && !gamepad2.a && gamepad1.cross) || gamepad1.cross || gamepad2.cross
        ...
        when (phase) {
            TestPhase.IDLE -> {
                if (IntakeAutoDatalog100Config.ENABLE_MANUAL_DRIVE_WHEN_IDLE) {
            ...
            TestPhase.EXTENDING_SLIDES -> {
                ...
                val isTimedOut = extensionTimer.seconds() >= IntakeAutoDatalog100Config.EXTENSION_TIMEOUT_SECONDS
            ...
            TestPhase.FORWARD_RUN -> {
                ...
                segmentRemainingMeters = max(0.0, IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS - segmentProgressMeters)

                val segmentTimeoutSec = max(IntakeAutoDatalog100Config.SEGMENT_TIMEOUT_SECONDS, (IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS / (IntakeAutoDatalog100Config.MAX_DRIVE_POWER * 0.6)) + 5.0)
                ...
                } else if (segmentProgressMeters >= IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS) {
            ...
            TestPhase.FORWARD_SETTLE -> {
                commandedForward = 0.0
                if (settleTimer.seconds() >= IntakeAutoDatalog100Config.SETTLE_TIME_SECONDS) {
            ...
            TestPhase.BACKWARD_SETTLE -> {
                commandedForward = 0.0
                if (settleTimer.seconds() >= IntakeAutoDatalog100Config.SETTLE_TIME_SECONDS) {
                    if (currentCycle < IntakeAutoDatalog100Config.TOTAL_CYCLES) {
                        currentCycle++
                        segmentIndex++
                        phase = TestPhase.FORWARD_RUN
                        recordSegmentStartTicks()
        ...
        if (datalogger.isLogging && driveTelemetry != null) {
            ...
            datalogger.writeRow(
                ...
                totalCycles = IntakeAutoDatalog100Config.TOTAL_CYCLES,
                segmentIndex = segmentIndex,
                segmentDirection = directionLabel,
                segmentTargetDistanceM = IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS,
        ...
        packet.put("Cycle", "$currentCycle/${IntakeAutoDatalog100Config.TOTAL_CYCLES}")
        packet.put("Segment", "$segmentIndex/${IntakeAutoDatalog100Config.TOTAL_CYCLES * 2}")
    ...
    private fun calculateProfilePower(
        ...
        val minP = IntakeAutoDatalog100Config.MIN_DRIVE_POWER
        val maxP = IntakeAutoDatalog100Config.MAX_DRIVE_POWER
        val rampUp = IntakeAutoDatalog100Config.RAMP_UP_DISTANCE_METERS
        val rampDown = IntakeAutoDatalog100Config.RAMP_DOWN_DISTANCE_METERS
        ...
        return directionSign * IntakeAutoDatalog100Config.FORWARD_DIRECTION_SIGN * speed
    }
    ...
    private fun beginTest() {
        ...
        segmentRemainingMeters = IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS
        abortReason = null
        ...
        datalogger.startLogging(prefix = "intake_3m_spd100_test")
    ...
    private fun renderTelemetry(drive: fgc.vietnam.robot01.Hardware.DriveTelemetry?) {
        val totalSegments = IntakeAutoDatalog100Config.TOTAL_CYCLES * 2
        when (phase) {
            TestPhase.IDLE -> {
                telemetry.addLine("[READY] Intake 3m Datalogger (Speed 1.00 Max)")
                telemetry.addData("Plan", "%d Cycles = %d Segments (%.1fm total)",
                    IntakeAutoDatalog100Config.TOTAL_CYCLES, totalSegments,
                    IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS * totalSegments
                )
            ...
            TestPhase.EXTENDING_SLIDES -> {
                ...
                telemetry.addData("Time", "%.2fs / %.1fs", extensionTimer.seconds(), IntakeAutoDatalog100Config.EXTENSION_TIMEOUT_SECONDS)
            ...
            TestPhase.FORWARD_RUN, TestPhase.BACKWARD_RUN -> {
                ...
                telemetry.addLine("[RUNNING] $dir | INTAKE ON (Spd 1.00 MAX)")
                telemetry.addData("Cycle", "%d / %d  |  Segment: %d / %d",
                    currentCycle, IntakeAutoDatalog100Config.TOTAL_CYCLES,
                    segmentIndex, totalSegments
                )
                telemetry.addData("Progress", "%.2f / %.2f m (%.1f%%)",
                    segmentProgressMeters, IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS,
                    (segmentProgressMeters / IntakeAutoDatalog100Config.TARGET_DISTANCE_METERS) * 100.0
                )
                if (drive != null) {
            ...
            TestPhase.FORWARD_SETTLE, TestPhase.BACKWARD_SETTLE -> {
                telemetry.addLine("[SETTLING] Pausing %.2fs between passes...".format(IntakeAutoDatalog100Config.SETTLE_TIME_SECONDS))
                telemetry.addData("Cycle", "%d / %d finished", currentCycle, IntakeAutoDatalog100Config.TOTAL_CYCLES)
            }

            TestPhase.COMPLETED -> {
                telemetry.addLine("[COMPLETE] Intake 3m Test (100% Speed) Finished Successfully!")
